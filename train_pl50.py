...
class TumourDataset(Dataset):
    ...
    CLASSES = ['Background', 'Normal brain', 'Leading edge', 'Infiltrating tumour', 'Tumour', 'Perinecrotic zone',
               'Necrosis', 'Hyperplastic vessels', 'Microvascular proliferation (MVP)', 'Vessels (not MVP)']
...
class TumourSegModel(pl.LightningModule):
    ...
    def __init__(self, arch, encoder_name, encoder_weights, in_channels, out_classes, **kwargs):
        ...
        # Set up loss here. Tversky loss was used for imbalanced dataset (https://www.researchgate.net/publication/334373086_Tversky_as_a_Loss_Function_for_Highly_Unbalanced_Image_Segmentation_using_3D_Fully_Convolutional_Deep_Networks)
...
if __name__ == '__main__':
    ...
    cp_callback = mc(dirpath='checkpoints/',
                     save_top_k=10,
                     monitor='val_loss_epoch',
                     save_on_train_epoch_end=True,
                     filename=f'{ARCHITECTURE}_{ENCODER}_{VERSION}_focalloss' + '_{epoch:02d}_{val_loss_epoch:.3f}'
                     )
